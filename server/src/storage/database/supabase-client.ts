...
function isLocalDatabase(): boolean {
  ...
  if (!url) return false;
  // 如果是 postgresql:// 开头，且不是 supabase.co，则认为是本地数据库
  return url.startsWith('postgresql://') && !url.includes('supabase.co');
}
...
function getSupabaseClient(token?: string): SupabaseClient {
  // 如果是本地数据库，返回一个模拟的客户端
  // 注意：本地模式下，应该使用 getLocalDb() 进行数据库操作
  if (isLocalDatabase()) {
    console.log('[DB] Using local PostgreSQL connection');
    // 返回一个空的 Supabase 客户端，实际数据库操作使用 db
...
export { loadEnv, getSupabaseCredentials, getSupabaseClient, getLocalDb, isLocalDatabase, pool };